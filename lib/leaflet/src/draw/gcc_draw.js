function updatePopup(e) {
    ...
    if (type === 'marker') {
        ...
        layer.bindPopup('<p><b>Cordenadas</b><br>GDA94, MGA Zone 55: ' + Math.round(j.x*1000)/1000 + ', '+Math.round(j.y*1000)/1000 + '<br/>WGS84 lat, long: ' + Math.round(ll.lat*10000)/10000 + ', '+Math.round(ll.lng*10000)/10000 + '</p>');
    }
    if(type ==='polyline') {
        length = lineLength(e.layer._latlngs);
        layer.bindPopup('tamanho é: '+length);            
    }
    if (type === 'polygon' || type === 'rectangle') {
        area = polygonArea(e.layer._latlngs);
        layer.bindPopup('Area é: '+area);
    }
...
{ 
  ...
  if (area > 100000)
  { 
    areaString = (Math.round(area/200,2)/10000).toString() + ' km²';
  }
  else
  {
    areaString = (Math.round(area/2)/1000).toString() + ' m²';
  }
  return  areaString;
}