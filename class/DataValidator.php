...
class DataValidator {
    ...
    protected function set_messages_default(){
        $this->_messages = array(
            ...
            'is_equals'      => 'O valor do campo %s deve ser igual ao valor do campo %s ',
    ...
    public function is_required(){
        if (empty($this->_data['value']) && $this->_data['value'] !== '0'){
    ...
    //criado especialmente para valor minimo de data
    public function min_value_date($value, $inclusive = false){
        $verify = ($inclusive === true ? !is_numeric($this->_data['value']) || $this->_data['value'] >= $value : !is_numeric($this->_data['value']) || $this->_data['value'] > $value);
        if (!$verify){
            $this->set_error(sprintf($this->_messages['min_value'], $this->_data['name'], date('d/m/Y', $value)));
        }
        return $this;
    }
    ...
    // criada especialmente para valor máximo de data
    public function max_value_date($value, $inclusive = false){
        $verify = ($inclusive === true ? !is_numeric($this->_data['value']) || $this->_data['value'] <= $value : !is_numeric($this->_data['value']) || $this->_data['value'] < $value);
        if (!$verify){
            $this->set_error(sprintf($this->_messages['max_value'], $this->_data['name'], date('d/m/Y', $value)));
        }
        return $this;
    }
     ...
     * @return DataValidator The self instance
     */
    public function is_equals($value, $identical = false, $nomeCampoIrmao){
        $verify = ($identical === true ? $this->_data['value'] === $value : strtolower($this->_data['value']) == strtolower($value));
        if(!$verify){
            $this->set_error(sprintf($this->_messages['is_equals'], $this->_data['name'], $nomeCampoIrmao));
        }
        return $this;
    }
